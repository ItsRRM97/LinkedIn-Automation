...
chmod +x "$ROOT/scripts/preflight_feed_engage.sh"
chmod +x "$ROOT/scripts/import_linkedin_cookies.sh"
chmod +x "$ROOT/scripts/notify_feed_engage_armed.sh"
chmod +x "$ROOT/scripts/trigger_feed_engage_agent.sh"
chmod +x "$ROOT/linkedin-feed-engage/feed_engage_daemon.py"
...
echo "  • feed_engage_daemon.py — hands-off feed comments (linkedincli read + Groq + Composio)"
echo ""
